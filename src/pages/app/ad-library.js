...
const store = reactive({
  ...
  startEditing(itemId) {
    ...
    setTimeout(() => {
      // Find the board item by looking for the container with the matching board name
      const boardItems = document.querySelectorAll('.cc_vue-for-swipefeed-boards');
      for (const item of boardItems) {
        const nameDiv = item.querySelector(`[v-text="item.name || 'Untitled Board'"]`);
        if (nameDiv) {
          const input = item.querySelector('.form_field.is--board.w-input');
          if (input) {
            input.focus();
            break;
          }
...
</svg></div><div>New Board</div></a></div></div><div class="divider"></div><div class="dropdown_padding"><div><div class="cc_vue-for-swipefeed-boards"><div v-bind:class="{ 'is--cc_saved': store.users_swipefeed.some(swipefeed => swipefeed.ad_id === ${hit.id} &amp;&amp; swipefeed.board_id === store.user.default_board_id &amp;&amp; !swipefeed.isDeleting) }" v-on:click="store.saveAdToBoard(${hit.id}, store.user.default_board_id)" class="cc_dropdown__list-item"><div class="h-flex-center-075 text-align-left"><div id="w-node-_9e4d3d4c-1cd4-80fa-1230-33f4618f07b3-618f0766" class="icon-20 w-embed"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg></div><div class="cc_swipefeed-toggle-inputs"><div class="heading-14">My SwipeFeed &nbsp;<span class="subtext-12">(Private)</span></div></div></div><div class="auto-left"><div>#</div></div></div></div><div class="divider bot5 top5"></div><div class="swipefeed-scroll"><div v-for="(item, index) in store.boards" class="cc_vue-for-swipefeed-boards"><div class="relative"><input v-focus="" class="form_field is--board w-input" maxlength="256" name="field-2" v-on:blur="store.updateBoardName(item.id, item.name)" data-name="Field 2" v-model="item.name" placeholder="Enter a board name..." v-if="store.editingBoardId === item.id" v-on:keyup="store.handleKeyup($event, item.id, item.name)" type="text" id="field-2" required=""><div v-bind:class="{ 'is--cc_white': store.users_swipefeed.some(swipefeed => swipefeed.ad_id === ${hit.id} &amp;&amp; swipefeed.board_id === item.id &amp;&amp; !swipefeed.isDeleting) }" class="cc_dropdown__list-item"><div class="h-flex-center-05 text-align-left"><div class="icon-20 relative-2 w-embed"><svg width="16" height="16" viewBox="0 0 84 84" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M4.66667 46.6667H32.6667C33.9043 46.6667 35.0913 46.175 35.9665 45.2998C36.8417 44.4247 37.3333 43.2377 37.3333 42V4.66667C37.3333 3.42899 36.8417 2.242 35.9665 1.36683C35.0913 0.491665 33.9043 0 32.6667 0H4.66667C3.42899 0 2.242 0.491665 1.36683 1.36683C0.491665 2.242 0 3.42899 0 4.66667V42C0 43.2377 0.491665 44.4247 1.36683 45.2998C2.242 46.175 3.42899 46.6667 4.66667 46.6667ZM0 79.3333C0 80.571 0.491665 81.758 1.36683 82.6332C2.242 83.5083 3.42899 84 4.66667 84H32.6667C33.9043 84 35.0913 83.5083 35.9665 82.6332C36.8417 81.758 37.3333 80.571 37.3333 79.3333V60.6667C37.3333 59.429 36.8417 58.242 35.9665 57.3668C35.0913 56.4917 33.9043 56 32.6667 56H4.66667C3.42899 56 2.242 56.4917 1.36683 57.3668C0.491665 58.242 0 59.429 0 60.6667V79.3333ZM46.6667 79.3333C46.6667 80.571 47.1583 81.758 48.0335 82.6332C48.9087 83.5083 50.0957 84 51.3333 84H79.3333C80.571 84 81.758 83.5083 82.6332 82.6332C83.5083 81.758 84 80.571 84 79.3333V46.6667C84 45.429 83.5083 44.242 82.6332 43.3668C81.758 42.4917 80.571 42 79.3333 42H51.3333C50.0957 42 48.9087 42.4917 48.0335 43.3668C47.1583 44.242 46.6667 45.429 46.6667 46.6667V79.3333ZM51.3333 32.6667H79.3333C80.571 32.6667 81.758 32.175 82.6332 31.2998C83.5083 30.4247 84 29.2377 84 28V4.66667C84 3.42899 83.5083 2.242 82.6332 1.36683C81.758 0.491665 80.571 0 79.3333 0H51.3333C50.0957 0 48.9087 0.491665 48.0335 1.36683C47.1583 2.242 46.6667 3.42899 46.6667 4.66667V28C46.6667 29.2377 47.1583 30.4247 48.0335 31.2998C48.9087 32.175 50.0957 32.6667 51.3333 32.6667Z" fill="currentColor"></path>
...
</svg></div><div>Delete </div></div></div></div><div v-bind:style="{    pointerEvents: store.loadingAdIds.has(${hit.id}) ? 'none' : 'auto',   opacity: store.loadingAdIds.has(${hit.id}) ? '0.5' : '1' }" v-on:click="store.saveAdToBoard(${hit.id}, item.id)" v-if="store.editingBoardId !== item.id" v-bind:class="{    'is--cc_saved': store.users_swipefeed.some(swipefeed => swipefeed.ad_id === ${hit.id} &amp;&amp; !swipefeed.isDeleting),   'is--disabled': store.loadingAdIds.has(${hit.id}) }" class="boards_clickable"></div></div></div></div></div></div></div></div></div></div></div></div></div></div>
        `},
        empty: `
