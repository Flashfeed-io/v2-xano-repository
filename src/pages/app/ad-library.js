...
const store = reactive({
  ...
  async saveAdToBoard(adId, boardId) {
    ...
    if (existingEntry) {
      ...
      // If unsaving from default board and using Instagram-style model,
      // mark all entries for this ad as deleting
      if (isDefaultBoard) {
    ...
    try {
      const response = await fetch('https://x6c9-ohwk-nih4.n7d.xano.io/api:DHN2-_b_/boards/save_ad', {
        ...
        body: JSON.stringify({
          ad_id: parseInt(adId),
          board_id: parseInt(boardId),
          remove_from_all: isDefaultBoard && existingEntry // If unsaving from default, remove from all
        })
      });
...
</svg></div><div class="pre">Rewrite</div></a></div></nav></div></div></div><div class="alg_ad-library_hit-padding is--video"><video controls="" width="100%" src="${hit.video_url}" class="video" draggable="true"></video></div><div class="alg_ad-library_hit-padding is--bottom"><div class="alg_save-hit-wrap"><div v-bind:class="{ 'is--cc_saved': store.users_swipefeed.some(item => item.ad_id === ${hit.id} && !item.isDeleting) }" cc_data="dropdown-button-wrap" class="cc_save-hit-wrap button-outline"><div cc_data="dropdown-button" class="alg_hit-save-left"><div class="button-icon w-embed"><svg width="100%" height="100%" viewBox="-1 -1 26 26" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path d="M5 7.8C5 6.11984 5 5.27976 5.32698 4.63803C5.6146 4.07354 6.07354 3.6146 6.63803 3.32698C7.27976 3 8.11984 3 9.8 3H14.2C15.8802 3 16.7202 3 17.362 3.32698C17.9265 3.6146 18.3854 4.07354 18.673 4.63803C19 5.27976 19 6.11984 19 7.8V21L12 17L5 21V7.8Z" fill="currentColor" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"></path>
</svg></div></div><div class="alg_hit-save-middle"><div v-text="store.users_swipefeed.some(item => item.ad_id === ${hit.id} && !item.isDeleting) ? 'Saved' : 'Save to SwipeFeed'">Save to SwipeFeed</div></div><div cc_data="dropdown-button" class="alg_hit-save-right"><div class="button-icon w-embed"><svg width="16" height="16" viewBox="0 0 84 84" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg></div><div class="alg_board-dropdown-arrow w-icon-dropdown-toggle"></div></div><div class="cc_option-dropdown__list-custom is--auto-left"><div class="dropdown_padding"><div class="dropdown_board-top-wrap"><div class="h-flex-center-075 text-align-left"><img src="https://cdn.prod.website-files.com/6643b76ff21fa0983192bff1/664495fa4d7615b45d74d1d3_home-icon.png" loading="lazy" alt="" class="icon_dashboard-nav"><div class="cc_swipefeed-toggle-inputs"><div class="heading-15">SwipeFeed Boards</div><div class="subtext-13">Save your ads to shareable boards.</div></div></div><a v-on:click="store.createNewBoard()" href="#" class="button-dark w-inline-block"><div class="button-icon w-embed"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M12 5V19M5 12H19" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg></div><div>New Board</div></a></div></div><div class="divider"></div><div class="dropdown_padding"><div><div class="cc_vue-for-swipefeed-boards">

<div v-bind:class="{ 'is--cc_saved': store.users_swipefeed.some(swipefeed => swipefeed.ad_id === ${hit.id} && swipefeed.board_id === store.user.default_board_id && !swipefeed.isDeleting) }" v-on:click="store.saveAdToBoard(${hit.id}, store.user.default_board_id)" class="cc_dropdown__list-item"><div class="h-flex-center-075 text-align-left"><div id="w-node-_9e4d3d4c-1cd4-80fa-1230-33f4618f07b3-618f0766" class="icon-20 w-embed"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg></div><div class="cc_swipefeed-toggle-inputs"><div class="heading-14 is--cc_board-default">My SwipeFeed</div></div></div><div class="auto-left"><div>#</div></div></div></div><div class="divider bot5 top5"></div><div v-for="(item, index) in store.boards" class="cc_vue-for-swipefeed-boards">

<div v-bind:class="{ 'is--cc_saved': store.users_swipefeed.some(swipefeed => swipefeed.ad_id === ${hit.id} && swipefeed.board_id === item.id && !swipefeed.isDeleting) }" v-on:click="store.saveAdToBoard(${hit.id}, item.id)" class="cc_dropdown__list-item">

<div class="h-flex-center-075 text-align-left"><div class="icon-20 relative-2 w-embed"><svg width="16" height="16" viewBox="0 0 84 84" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M4.66667 46.6667H32.6667C33.9043 46.6667 35.0913 46.175 35.9665 45.2998C36.8417 44.4247 37.3333 43.2377 37.3333 42V4.66667C37.3333 3.42899 36.8417 2.242 35.9665 1.36683C35.0913 0.491665 33.9043 0 32.6667 0H4.66667C3.42899 0 2.242 0.491665 1.36683 1.36683C0.491665 2.242 0 3.42899 0 4.66667V42C0 43.2377 0.491665 44.4247 1.36683 45.2998C2.242 46.175 3.42899 46.6667 4.66667 46.6667ZM0 79.3333C0 80.571 0.491665 81.758 1.36683 82.6332C2.242 83.5083 3.42899 84 4.66667 84H32.6667C33.9043 84 35.0913 83.5083 35.9665 82.6332C36.8417 81.758 37.3333 80.571 37.3333 79.3333V60.6667C37.3333 59.429 36.8417 58.242 35.9665 57.3668C35.0913 56.4917 33.9043 56 32.6667 56H4.66667C3.42899 56 2.242 56.4917 1.36683 57.3668C0.491665 58.242 0 59.429 0 60.6667V79.3333ZM46.6667 79.3333C46.6667 80.571 47.1583 81.758 48.0335 82.6332C48.9087 83.5083 50.0957 84 51.3333 84H79.3333C80.571 84 81.758 83.5083 82.6332 82.6332C83.5083 81.758 84 80.571 84 79.3333V46.6667C84 45.429 83.5083 44.242 82.6332 43.3668C81.758 42.4917 80.571 42 79.3333 42H51.3333C50.0957 42 48.9087 42.4917 48.0335 43.3668C47.1583 44.242 46.6667 45.429 46.6667 46.6667V79.3333ZM51.3333 32.6667H79.3333C80.571 32.6667 81.758 32.175 82.6332 31.2998C83.5083 30.4247 84 29.2377 84 28V4.66667C84 3.42899 83.5083 2.242 82.6332 1.36683C81.758 0.491665 80.571 0 79.3333 0H51.3333C50.0957 0 48.9087 0.491665 48.0335 1.36683C47.1583 2.242 46.6667 3.42899 46.6667 4.66667V28C46.6667 29.2377 47.1583 30.4247 48.0335 31.2998C48.9087 32.175 50.0957 32.6667 51.3333 32.6667Z" fill="currentColor"></path>
</svg></div><div class="cc_swipefeed-toggle-inputs"><div v-text="item.name || 'Untitled Board'" v-if="store.editingBoardId !== item.id" v-on:click="store.startEditing(item.id)" class="heading-14 is--cc_board-pointer">Lorem ipsum dolor sit amet.</div><input v-focus="" class="form_field is--board w-input" maxlength="256" name="field-2" v-on:blur="store.updateBoardName(item.id, item.name)" data-name="Field 2" v-else="" v-model="item.name" placeholder="Enter a board name..." v-on:keyup="store.handleKeyup($event, item.id, item.name)" type="text" id="field-2" required=""></div></div><div class="auto-left"><div v-if="store.editingBoardId !== item.id" class="relative-2">#</div><div v-on:click="store.deleteBoard(item.id)" v-else="" class="button-outline is--small-version is--delete"><div class="button-icon w-embed"><svg width="20" height="20" viewBox="-1 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
