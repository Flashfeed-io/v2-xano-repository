...
$(document).ready(function () {
  ...
  function toggleVisibility() {
    if (store.fields.workspace === "Under Review") {
      ...
      $("#cc_startDate").attr("required", true);
      $("#cc_deliveryDate").attr("required", true);
    } else {
      $('[public-enabled="false"]').show();
      $('[public-enabled="true"]').hide();

      // Remove required attribute when not under review
      $("#cc_startDate").removeAttr("required");
      $("#cc_deliveryDate").removeAttr("required");
    }
  }
...
MemberStack.onReady.then(function (member) {
  console.log("vue memberstack member object vUserData", vUserData);
  console.log("vue memberstack member id", member.id);
  store.fields.memberstack_id = member.id;
});
  ...
  const response = await fetch(
    "https://x8ki-letl-twmt.n7.xano.io/api:ZUu16G-Q/create-workspace",
    {
      ...
      body: JSON.stringify({
        ...
        inputAspectRatio: store.storyboardAspectRatio,
...
const mounted = async () => {
  ...
  editorBrandTopicsToHighlight.on(
    "text-change",
    function (delta, oldDelta, source) {
