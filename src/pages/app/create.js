...
*/
    ...
    $("[cc_data=cloned-duration]").text(cloneLength);
...
const store = reactive({
  ...
  fields: {
    ...
    storyboardJSON: "",
    storyboardJobId: "",
    storyboardAspectRatio: "1024x1024",
...
async function userAnalytics(
  ...
  const response = await fetch(
    `https://x6c9-ohwk-nih4.n7d.xano.io/api:QLDf95Yy/user-analytics/` +
      store.member_id,
    {
      method: "PUT",
      headers: {
...
const getCreativeStyles = async () => {
  console.log("Checking -> : inside getCreativeStyles 1");
  const response = await fetch(
  ...
  console.log("Checking -> : inside getCreativeStyles 2");
  if (!response.ok) {
    const message = `An error has occured: ${response.status}`;
    throw new Error(message);
  }
  console.log("Checking -> : inside getCreativeStyles 3");
  const data = await response.json();
  return data;
};
...
const analyzeVideo = async (event) => {
  ...
  const response = await fetch(
    "https://x6c9-ohwk-nih4.n7d.xano.io/api:asT3vkWc/import",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json", // This line is important
...
const checkPingImport = async (job_id) => {
  ...
  const response = await fetch(
    "https://x6c9-ohwk-nih4.n7d.xano.io/api:asT3vkWc/import/ping/" + job_id,
    {
      method: "GET",
    }
  );
...
const researchInsights = async () => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const response = await fetch(
      "https://x6c9-ohwk-nih4.n7d.xano.io/api:ZUu16G-Q/insights",
      {
        method: "POST",
        headers: {
...
const rateMyConcept = async (event) => {
  ...
  try {
    const response = await fetch(
      "https://x6c9-ohwk-nih4.n7d.xano.io/api:ZUu16G-Q/rate-concept",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const importFromUrl = async (event) => {
  store.isImportingFromUrl = true;
  console.log(event);

  try {
    const response = await fetch(
      "https://x6c9-ohwk-nih4.n7d.xano.io/api:ZUu16G-Q/url",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          inputUrl: store.importedUrl,
        }),
      }
    );

    const data = await response.json();
    console.log("jsondata for importFromUrl", data);

    store.fields.brand_name = data.brandTitle;
    let textToReplace = data.brandDescription;

    // Convert newlines to <br> tags to preserve formatting in Quill
    textToReplace = textToReplace.replace(/\n/g, "<br>");

    // Insert HTML content into Quill editor using innerHTML to preserve HTML formatting
    editorBrandServiceOrProduct.root.innerHTML = textToReplace;
  } catch (error) {
    console.error("Error in importFromUrl", error);
  } finally {
    store.isImportingFromUrl = false;
  }
};
...
const regenerateStoryboard = async (event) => {
  ...
  const response = await fetch(
    "https://x6c9-ohwk-nih4.n7d.xano.io/api:IY_ab71y/storyboards/regenerate",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json", // This line is important
  ...
  if (!response.ok) {
    store.isAIWorking = false;
    updateShotStatus(store.storyboardJSON, sceneNumber, shotNumber, "");
  }
  if (response.ok) {
    const data = await response.json(); // Parse the JSON in the response
    console.log("Updated shot object:", data);

    // Only update the specific shot in the storyboard JSON
    updateShot(store.storyboardJSON, sceneNumber, shotNumber, data);

    store.isAIWorking = false;
  }
};

function updateShot(storyboard, sceneNumber, shotNumber, newData) {
  storyboard.forEach((scene) => {
    if (scene.sceneNumber === sceneNumber) {
      scene.shotElements.forEach((shot) => {
        if (shot.shotNumber === shotNumber) {
          // Update the shot with new data
          Object.assign(shot, newData);
        }
      });
    }
  });
...
const cancelRegenStoryboard = async (event) => {
  // Extract the event handler button and find the attributes to send their values as the inputs for the fetch
  const button = event.target;
  console.log("button", button);
  const sceneNumber = button.getAttribute("sceneNumber");
  console.log("sceneNumber", sceneNumber);
  const shotNumber = button.getAttribute("shotNumber");
  console.log("shotNumber", shotNumber);

  const storyboard = store.fields.storyboardJSON;

  cancelShotStatus(storyboard, sceneNumber, shotNumber);
};

// Define the updateShotStatus function
function cancelShotStatus(storyboard, sceneNumber, shotNumber) {
  // Loop through each scene in the storyboard
  storyboard.forEach((scene) => {
    // Check if the scene matches the provided sceneNumber
    if (scene.sceneNumber == sceneNumber) {
...
const createWorkspace = async () => {
  ...
  const response = await fetch(
    "https://x6c9-ohwk-nih4.n7d.xano.io/api:ZUu16G-Q/create-workspace",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
const checkPingWorkspace = async (job_id) => {
  ...
  const response = await fetch(
    "https://x6c9-ohwk-nih4.n7d.xano.io/api:IY_ab71y/storyboards/ping/" +
      job_id,
    {
      method: "GET",
    }
...
const generateStoryboard = async () => {
  ...
  const response = await fetch(
    "https://x6c9-ohwk-nih4.n7d.xano.io/api:IY_ab71y/storyboards/create",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
const checkPingStoryboard = async (job_id) => {
  ...
  if (data.string_status == "Thinking") {
    ...
    setTimeout(() => {
      ...
      console.log("rerunning checkPingStoryboard function for thinking");
      store.storyboardJSON = data.storyboard_json;
      store.fields.storyboardJSON = data.storyboard_json;
      console.log("current storyboard_json:", data.storyboard_json); // Access the job_id from the parsed data
    }, 1500);
  } else if (data.string_status == "Generating") {
    store.storyboardStatus = "Generating";
...
const mounted = async () => {
  ...
  console.log("Checking -> : creativeStylesStore", store.creativeStyles);
  store.creativeStyles = await getCreativeStyles();
  console.log("Checking -> : creativeStylesStore 2", store.creativeStyles);
...
const backToForm = () => {
  // Select the element with the class 'w-form-done' and set its display to 'none'
  const formDoneElement = document.querySelector(".w-form-done");
  if (formDoneElement) {
    formDoneElement.style.display = "none";
  }

  // Select the element with the name attribute 'wf-form-Base-Component-Form' and set its display to 'block'
  const formBaseComponent = document.querySelector(
    '[name="wf-form-Base-Component-Form"]'
  );
  if (formBaseComponent) {
    formBaseComponent.style.display = "block";
  }
};
